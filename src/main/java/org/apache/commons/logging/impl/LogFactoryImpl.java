...
public class LogFactoryImpl extends LogFactory {
    ...
    public Log getInstance(String name) throws LogConfigurationException {
        //从Hashtable中，根据类名称获取，对应日志实例
        Log instance = (Log) instances.get(name);
        //如果不存在，则进行实例化，再保存到Hashtable中：
        if (instance == null) {
...
//        如果commons-logging.properties文件中不存在“org.apache.commons.logging.Log”的value：
//        那么还有遍历classesToDiscover字符串数组，进行实例化：
//        此数组中包括：log4j、Jdk14Logger、Jdk13LumberjackLogger、SimpleLog
//        首先会实例化Log4JLogger对象，如果没有引入log4j的jar包，程序会报错，则进行Jdk14Logger的初始化；
